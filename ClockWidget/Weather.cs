...
internal static class Weather
{
    ...
    private static void NotifyListeners(WeatherData current)
    {
        ...
        foreach (var handler in Listeners) handler(current);
    }
    ...
    {
        if (_canUpdateWeatherData) return;
    ...
    private static async Task<MeteoData?> GetMeteoData(double latitude, double longitude)
    {
        ...
        return await client.GetFromJsonAsync<MeteoData>(
            $"https://api.open-meteo.com/v1/forecast?latitude={latitude.ToString("F4", culture)}&longitude={longitude.ToString("F4", culture)}&current=temperature_2m");
    }
    ...
    private static async Task PeriodicallyUpdateData()
    {
        ...
        {
            var ip = await GetIP();
            if (ip == ERROR_DETECTING_IP) return;

            var geoData = await GetGeoData(ip);
            if (geoData?.status != "success" || geoData?.lat == null || geoData?.lon == null || geoData?.city == null ||
                geoData?.countryCode == null) return;

            var meteoData = await GetMeteoData(geoData.lat, geoData.lon);
            if (meteoData?.current?.temperature_2m == null) return;

            CurrentData = new WeatherData(geoData.city, geoData.countryCode, meteoData.current.temperature_2m, DateTime.Now);
